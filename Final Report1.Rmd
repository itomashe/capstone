...
Data Transformation script was mostly following 1. The data set is too big to handle in R, so we applied the below sripts just to news data set. 
It could be easily expanded to other data sets. 
```{r warning=FALSE}
...
````
At this point we have enough data to calculate the maximum likelihood estimate of the probability of N-gram based on the news data.
For example:  
...
Backoff is another method for dealing with unseen n-gram.
...
If n-gram has appered more than k times(k is set to 0), then an n-fram estimate is used, if n-gram did not appear then we will use an estimate from a shorter n-gram. This recursion can continue down, so that we can start with a trigram model and end up estimating the next word based on unigram frequncies.
Such models are simple and in practice work well.
